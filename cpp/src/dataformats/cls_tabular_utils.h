...
class TypedPredicate : public PredicateBase {
    ...
    public:
        TypedPredicate(int idx, int type, int op, const T& val, const int ch_op=SOT_logical_and) :
            ...
            col_type(type),
            op_type(op),
            is_global_agg(op==SOT_min || op==SOT_max || op==SOT_sum || op==SOT_cnt),
            value(val),
...
inline bool compareColInfo(const struct col_info& l, const struct col_info& r) {
    return (
        (l.idx==r.idx) &&
        (l.type==r.type) &&
        (l.is_key==r.is_key) &&
...
int extract_arrow_from_string(std::shared_ptr<arrow::Table>* table, const char *ds, int ds_size);

int convert_arrow_to_buffer(const std::shared_ptr<arrow::Table> &table, std::shared_ptr<arrow::Buffer>* buffer);

int split_arrow_table(std::shared_ptr<arrow::Table> &table, int max_rows, std::vector<std::shared_ptr<arrow::Table>>* table_vec);
}
